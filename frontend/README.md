...
The UI follows the official shadcn blocks (`dashboard-01` shell, `sidebar-07` navigation, `login-03`) on the stock neutral theme; avoid custom gradients/glass effects. Add components with `npx shadcn@latest add <name>`, or via the shadcn MCP server configured in the repo-root `.mcp.json` (restart Claude Code to load it). Brand name and tagline live in `src/config/brand.ts`.
...
src/
  ...
  features/office/       live office: layout (walkway graph), Character, Furniture, director (GSAP behaviour), OfficeFloor
